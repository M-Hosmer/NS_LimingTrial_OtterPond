...
ps.merged = tax_glom(physeq_rclr, taxrank=rnk, NArm=TRUE)
...
ancom_out_treat_hori_up_ff = ANCOM(process$feature_table, process$meta_data, process$structure_zeros, main_var='Groups')
saveRDS(ancom_out_treat_hori_up_ff, paste(folder, "R_objects_new/ancom_out_treat_hori_up_ff.rds", sep=""))
...
process = feature_table_pre_process(ft, md, 'Samples', 'Groups', lib_cut=10, neg_lb=TRUE)
ancom_out_treat_hori_low_ff = ANCOM(process$feature_table, process$meta_data, process$structure_zeros, main_var='Groups')
saveRDS(ancom_out_treat_hori_low_ff, paste(folder, "R_objects_new/ancom_out_treat_hori_low_ff.rds", sep=""))
a <- ifelse(ancom_out_treat_hori_low_ff$out$detected_0.9 == TRUE, "red", "black")
```
...
process = feature_table_pre_process(ft, md, 'Samples', 'Groups', lib_cut=10, neg_lb=TRUE)
ancom_out_treat_hori_uB = ANCOM(process$feature_table, process$meta_data, process$structure_zeros, main_var='Groups')
saveRDS(ancom_out_treat_hori_uB, paste(folder, "R_objects_new/ancom_out_treat_hori_uB.rds", sep=""))
a <- ifelse(ancom_out_treat_hori_uB$out$detected_0.9 == TRUE, "red", "black")
```
...
###ancom merge
```{R, results='hide', fig.keep='all', message=FALSE}
ancom_out_treat_hori_up_ff = readRDS(file =  paste(folder, "R_objects_new/ancom_out_treat_hori_up_ff.rds", sep=""))
ancom_out_treat_hori_low_ff = readRDS(file =  paste(folder, "R_objects_new/ancom_out_treat_hori_low_ff.rds", sep=""))
ancom_out_treat_hori_uB = readRDS(file =  paste(folder, "R_objects_new/ancom_out_treat_hori_uB.rds", sep=""))
rnk = "ta6"
ps.rank = tax_glom(physeq_rclr, taxrank=rnk, NArm=TRUE)
ft = otu_table(ps.rank)
tax = tax_table(ps.rank)
rownames(ft) = tax[,numrnk]
taxa_names(ps.rank) = rownames(ft)
rank.sum = tapply(taxa_sums(ps.rank), rownames(tax_table(ps.rank)), sum, na.rm=TRUE)
top100 = names(sort(rank.sum, TRUE))[1:100]
ps.rank = prune_taxa((rownames(tax_table(ps.rank)) %in% top100), ps.rank)
ps.filter = merge_samples(ps.rank, "Treatment_Location_Horizon")
otu_table(ps.filter)
saveRDS(ps.filter, file = paste(folder, "R_objects_new/ps.filter.rds", sep=""))
...
taxa_names(ps.rank) = rownames(ft)
ps.rank = prune_taxa((rownames(tax_table(ps.rank)) %in% top100), ps.rank)
asv = otu_table(ps.rank)
write.csv(asv, paste(folder, "R_objects_new/asv_raw_treatment_horizon_separate_samples.csv", sep=""))
ps.filter = merge_samples(ps.rank, "Treatment_Location_Horizon")
asv = otu_table(ps.filter)
write.csv(asv, paste(folder, "R_objects_new/asv_raw_treatment_horizon_filtered.csv", sep=""))


up_ff = ancom_out_treat_hori_up_ff$out[, c(1,3)]
rownames(up_ff) = up_ff[,1]
colnames(up_ff) = c('taxa_id', 'Upper Forest Floor')

low_ff = ancom_out_treat_hori_low_ff$out[, c(1,3)]
rownames(low_ff) = low_ff[,1]
colnames(low_ff) = c('taxa_id', 'Lower Forest Floor')

uB = ancom_out_treat_hori_uB$out[, c(1,3)]
rownames(uB) = uB[,1]
colnames(uB) = c('taxa_id', 'Upper B Horizon')

merged = merge(up_ff, low_ff, by=0, all=TRUE)
rownames(merged) = merged[,1]
merged = merge(merged, uB, by=0, all=TRUE)
rownames(merged) = merged[,1]
merged = merged[,c(4,6,8)]
merged[is.na(merged)] <- FALSE

saveRDS(merged, file = paste(folder, "R_objects_new/merged_ancom_horizon_treatment.rds", sep=""))
write.csv(merged,  paste(folder, "R_objects_new/merged_ancom_horizon_treatment.csv", sep=""))
```

###ALDEx merge 
```{R, results='hide', fig.keep='all', message=FALSE}
ALDEx_Upper_Forest_Floor = readRDS(file =  paste(folder, "R_objects_new/ALDEx_Upper_Forest_Floor.rds", sep=""))
ALDEx_Lower_Forest_Floor = readRDS(file =  paste(folder, "R_objects_new/ALDEx_Lower_Forest_Floor.rds", sep=""))
ALDEx_Upper_B = readRDS(file =  paste(folder, "R_objects_new/ALDEx_Upper_B.rds", sep=""))


write.csv(ALDEx_Upper_Forest_Floor, paste(folder, "R_objects_new/ALDEx_Upper_Forest_Floor.csv", sep=""))
write.csv(ALDEx_Lower_Forest_Floor, paste(folder, "R_objects_new/ALDEx_Lower_Forest_Floor.csv", sep=""))
write.csv(ALDEx_Upper_B, paste(folder, "R_objects_new/ALDEx_Upper_B.csv", sep=""))
```

Re-filter ANCOM:
```{R, eval=FALSE}
da_folder = '/Users/maggiehosmer/Library/CloudStorage/OneDrive-DalhousieUniversity/Acid_Rain_Project/March_23/robyn_analysis/maggie_analysis_files/differential_abundance_16S/'
...
all_da.to_csv(folder+'robyn_analysis/differential_abundance_intermediate/all_da_16S.csv')
```
...
tree = Phylo.read(folder+'robyn_analysis/differential_abundance_intermediate/reduced_tree_16S.tree', "newick")
...
ft_ra = pd.read_csv(folder+'robyn_analysis/differential_abundance_intermediate/ft_ra_red_16S.csv', index_col=0, header=0)
...
ft_rclr = pd.read_csv(folder+'robyn_analysis/differential_abundance_intermediate/ft_rclr_red_16S.csv', index_col=0, header=0)
...
da_int_folder = folder+'robyn_analysis/differential_abundance_intermediate/'
...
plt.savefig(folder+'robyn_analysis/figures/tree_heatmap_16S.png', dpi=600, bbox_inches='tight')
```

Now make boxplots with the top taxa:
```{python}
top_taxa = pd.read_csv(folder+'robyn_analysis/differential_abundance_intermediate/ft_ra_red_16S.csv', index_col=0, header=0).index.values
...
for tax in top_taxa:
  ...
  plt.savefig('/Users/robynwright/Dropbox/Langille_Lab_postdoc/Acid_rain_recovery/robyn_analysis/figures/boxplots/16S_taxa_'+str(count+1)+'_'+tax+'.png', dpi=600, bbox_inches='tight')
  count += 1
...
process = feature_table_pre_process(ft, md, 'Samples', 'Groups', lib_cut=10, neg_lb=TRUE)
ancom_out_treat_hori_up_ff = ANCOM(process$feature_table, process$meta_data, process$structure_zeros, main_var='Groups')
saveRDS(ancom_out_treat_hori_up_ff, paste(folder, "R_objects_ITS/ancom_out_treat_hori_up_ff.rds", sep=""))
a <- ifelse(ancom_out_treat_hori_up_ff$out$detected_0.9 == TRUE, "red", "black")
```

#### Lower Forest Floor Layer 
...
process = feature_table_pre_process(ft, md, 'Samples', 'Groups', lib_cut=10, neg_lb=TRUE)
ancom_out_treat_hori_low_ff = ANCOM(process$feature_table, process$meta_data, process$structure_zeros, main_var='Groups')
saveRDS(ancom_out_treat_hori_low_ff, paste(folder, "R_objects_ITS/ancom_out_treat_hori_low_ff.rds", sep=""))
a <- ifelse(ancom_out_treat_hori_low_ff$out$detected_0.9 == TRUE, "red", "black")
```
...
process = feature_table_pre_process(ft, md, 'Samples', 'Groups', lib_cut=10, neg_lb=TRUE)
ancom_out_treat_hori_uB = ANCOM(process$feature_table, process$meta_data, process$structure_zeros, main_var='Groups')
saveRDS(ancom_out_treat_hori_uB, paste(folder, "R_objects_ITS/ancom_out_treat_hori_uB.rds", sep=""))
a <- ifelse(ancom_out_treat_hori_uB$out$detected_0.9 == TRUE, "red", "black")
```
...
###ancom merge
```{R, results='hide', fig.keep='all'}
ancom_out_treat_hori_up_ff = readRDS(file =  paste(folder, "R_objects_ITS/ancom_out_treat_hori_up_ff.rds", sep=""))
ancom_out_treat_hori_low_ff = readRDS(file =  paste(folder, "R_objects_ITS/ancom_out_treat_hori_low_ff.rds", sep=""))
ancom_out_treat_hori_uB = readRDS(file =  paste(folder, "R_objects_ITS/ancom_out_treat_hori_uB.rds", sep=""))
...
ps.filter = merge_samples(ps.rank, "Treatment_Location_Horizon")
otu_table(ps.filter)
saveRDS(ps.filter, file = paste(folder, "R_objects_ITS/ps.filter.rds", sep=""))
# 
rnk = "ta6"
ps.rank = tax_glom(physeq, taxrank=rnk, NArm=TRUE)
ft = otu_table(ps.rank)
...
ps.filter = merge_samples(ps.rank, "Treatment_Location_Horizon")
asv = otu_table(ps.filter)
write.csv(asv, paste(folder, "R_objects_ITS/asv_raw_treatment_horizon_filtered.csv", sep=""))
...
saveRDS(merged, file = paste(folder, "R_objects_ITS/merged_ancom_horizon_treatment.rds", sep=""))
write.csv(merged,  paste(folder, "R_objects_ITS/merged_ancom_horizon_treatment.csv", sep=""))
```

###ALDEx merge 
```{R, results='hide', fig.keep='all', message=FALSE}
ALDEx_Upper_Forest_Floor = readRDS(file =  paste(folder, "R_objects_ITS/ALDEx_Upper_Forest_Floor.rds", sep=""))
ALDEx_Lower_Forest_Floor = readRDS(file =  paste(folder, "R_objects_ITS/ALDEx_Lower_Forest_Floor.rds", sep=""))
ALDEx_Upper_B = readRDS(file =  paste(folder, "R_objects_ITS/ALDEx_Upper_B.rds", sep=""))


write.csv(ALDEx_Upper_Forest_Floor, paste(folder, "R_objects_ITS/ALDEx_Upper_Forest_Floor.csv", sep=""))
write.csv(ALDEx_Lower_Forest_Floor, paste(folder, "R_objects_ITS/ALDEx_Lower_Forest_Floor.csv", sep=""))
write.csv(ALDEx_Upper_B, paste(folder, "R_objects_ITS/ALDEx_Upper_B.csv", sep=""))
...
Re-filter ANCOM:
```{R}
da_folder = '/Users/maggiehosmer/Library/CloudStorage/OneDrive-DalhousieUniversity/Acid_Rain_Project/March_23/robyn_analysis/maggie_analysis_files/differential_abundance_ITS/'
low_ff = readRDS(paste(da_folder, 'ancom_out_treat_hori_low_ff.rds', sep=''))$out
up_ff = readRDS(paste(da_folder, 'ancom_out_treat_hori_up_ff.rds', sep=''))$out
up_B = readRDS(paste(da_folder, 'ancom_out_treat_hori_uB.rds', sep=''))$out
write.csv(low_ff, paste(da_folder, 'ancom_low_ff.csv', sep=''))
...
ancom = pd.concat(ancom).fillna(value=0)
ancom = ancom.groupby(by=ancom.index, axis=0).sum()
ancom.to_csv(folder+'robyn_analysis/differential_abundance_intermediate/ancom_ITS.csv')
...
aldex = pd.concat(aldex).fillna(value=0)
aldex = aldex.groupby(by=aldex.index, axis=0).sum()
aldex.to_csv(folder+'robyn_analysis/differential_abundance_intermediate/aldex_ITS.csv')
...
maaslin = pd.concat(maaslin).fillna(value=0)
maaslin = maaslin.groupby(by=maaslin.index, axis=0).sum()
maaslin.to_csv(folder+'robyn_analysis/differential_abundance_intermediate/maaslin_ITS.csv')

all_da = pd.concat([ancom, aldex, maaslin]).fillna(value=0)
all_da = all_da.groupby(by=all_da.index, axis=0).sum()
all_da.to_csv(folder+'robyn_analysis/differential_abundance_intermediate/all_da_ITS.csv')
```
...
ft_ra = ft_ra.loc[all_gen, :]
ft_rclr = ft_rclr.loc[all_gen, :]
ft_ra.to_csv(folder+'robyn_analysis/differential_abundance_intermediate/ft_ra_red_ITS.csv')
ft_rclr.to_csv(folder+'robyn_analysis/differential_abundance_intermediate/ft_rclr_red_ITS.csv')
```
...
ft_ra = pd.read_csv(folder+'robyn_analysis/differential_abundance_intermediate/ft_ra_red_ITS.csv', index_col=0, header=0)
ft_ra['Taxonomy'] = ''
...
ft_rclr = pd.read_csv(folder+'robyn_analysis/differential_abundance_intermediate/ft_rclr_red_ITS.csv', index_col=0, header=0)
ft_rclr = ft_rclr.loc[order, :].rename(columns=samples_treat_loc_only)
treat_order = ['Upper Forest Floor Control', 'Upper Forest Floor Treatment', 'Lower Forest Floor Control', 'Lower Forest Floor Treatment', 'Upper B Horizon Control', 'Upper B Horizon Treatment']
ft_rclr = ft_rclr.groupby(by=ft_rclr.columns, axis=1).mean().loc[:, treat_order]
plt.sca(ax_rclr)
...
plt.subplots_adjust(hspace=0.7)
plt.savefig(folder+'robyn_analysis/figures/tree_heatmap_ITS.png', dpi=600, bbox_inches='tight')
```

Now make boxplots with the top taxa:
```{python}
top_taxa = pd.read_csv(folder+'robyn_analysis/differential_abundance_intermediate/ft_ra_red_ITS.csv', index_col=0, header=0).index.values
...
for tax in top_taxa:
  ...
  for a in range(len(axes)):
    ...
    for b in range(len(treat_order)):
      ...
      for c in range(len(x_loc_hor)):
        tx = axes[a].text(x_loc_hor[c], -0.2, x_lab_hor[c], ha='center', va='center', transform=axes[a].transAxes)
  plt.savefig('/Users/robynwright/Dropbox/Langille_Lab_postdoc/Acid_rain_recovery/robyn_analysis/figures/boxplots/ITS_taxa_'+str(count+1)+'_'+tax+'.png', dpi=600, bbox_inches='tight')
  count += 1
